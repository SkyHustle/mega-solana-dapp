...
export default async function Pools() {
  ...
  // NOTE: Pulls all the banks from on-chain data during initialization
  // After this initial setup, banks can be refreshed by calling `await client.group.reload()`
  ...
  const SAMPLE_WALLET = new PublicKey(
    "FHsALmky5sQLUx6CrjWRLRfc4WQYjv4ecmWML3eLLAB1" // 1 account - size
    // "2ZJz3sKxpUNrTcAdSbeR1HqXCutdCvT9Jd2hpVMK5jMU" // 2 accounts
    // "5kdkhe5XZMmU4snHYSV42rn42UkxNnqqimF9pmk1CJjR" // 1 account
  );

  const marginfiAccounts = await client.getMarginfiAccountsForAuthority(SAMPLE_WALLET); // Fetch all user accounts under that wallet (there can be any number)
